...
public class PractitionerServiceDelegate {
    ...
    public void init() {
        ...
        practitioners.add(new Practitioner(2, "Dr. Jane Smith", "General Physician"));
        practitioners.add(new Practitioner(3, "Dr. Chuck Berry", "Cardiologist"));
        practitioners.add(new Practitioner(4, "Dr. Stu Pendous", "Neurologist"));
        practitioners.add(new Practitioner(5, "Dr. Mike Rosoft", "Psychiatrist"));
    }
    ...
    public Practitioner getPractitionerById(long id) {
        return practitioners.stream().filter(p -> p.getId() == id).findFirst().orElse(null);
    }
    ...
    public Practitioner updatePractitioner(long id, Practitioner practitioner) {
        Practitioner existingPractitioner = practitioners.stream().filter(p -> p.getId() == id).findFirst()
                .orElse(null);
    ...
    public void addConsultation(long id, long idPatientS, String date) {
        restTemplate.postForObject(
                "http://localhost:8095/appointments/createConsultation?patientId=" + idPatientS + "&practitionerId="
                        + id + "&date=" + date,
                null,
                String.class);
    }
    ...
    public List<String> getConsultation(long id) {
        HttpEntity<String> entity = new HttpEntity<>(null, null);
        return restTemplate.exchange(
                "http://localhost:8095/appointments/consultations/practitioner/" + id,
                HttpMethod.GET,
                entity,
